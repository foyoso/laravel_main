...
class ListingTypeService
{
    ...
    public function getAllForSelectBox($userId){
        return ListingType::where('user_id', $userId) -> select('id','name') -> orderbyDesc('updated_at')->get();
    }
    public function create($request)
    {
        DB::beginTransaction();
        try {
            $listing = ListingType::create([
    ...
    public function delete($request)
    {
        try{
            ...
            $post = ListingType::where('id', $id)->first();
            if ($post) {
